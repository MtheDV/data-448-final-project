...
const TeamAnalysisDetails = ({analysis, team, setSelectedTeamFromAnalysis}: AnalysisProps) => {
  ...
  return (
    <div>
      ...
      <button
        className={`text-left p-3 border-2 w-full ${analysis.type === analysisTypeNegative ? 'border-red-400' : analysis.type === analysisTypePositive ? 'border-green-600' : 'border-gray-400'} rounded-lg hover:bg-gray-100 hover:shadow`}
        onClick={setSelected}
      >
