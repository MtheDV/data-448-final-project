          ...
          <ul className={'flex flex-col gap-4'}>
            {analysis.details.filter(detail => detail.type !== analysisTypeNeutral).map((detail, index) =>
              <li key={`analysis-details-${index}`}>
                <button
                  className={`text-left p-3 border-2 ${detail.type === analysisTypeNegative ? 'border-red-400' : detail.type === analysisTypePositive ? 'border-green-600' : 'border-gray-400'} rounded-lg hover:bg-gray-100 hover:shadow`}
                  onClick={setSelected}
                >
                  <p>{detail.results}</p>
                </button>
